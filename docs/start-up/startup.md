...
将 `dud=ftp://ftp.example.com/PATH_TO_DRIVER` 或 `dud=http://www.example.com/PATH_TO_DRIVER` 设置为从 URL 加载驱动程序。当 `dud=1` 时，您将在启动时被要求提供 URL。
...
2.  将 `proxy` 参数附加到以下格式的 `linux` 行
    ...
    ![GRUB options editor](./image/grub_cmdline_proxy_osuse.png]](./image/grub_cmdline_proxy_osuse.png)
...
![Language, keyboard and product selection](./image/install_welcome_osuse.png]](./image/install_welcome_osuse.png)
...
单击 ![更改小部件主题](./image/yast-day-night-mode.svg) 按钮或按 Shift–F3 打开主题选择对话框。从列表中选择一个主题并 关闭 对话框。
...
![Network Settings](./image/install_network_osuse.png]](./image/install_network_osuse.png)
...
![System Role](./image/install_ui_osuse.png]](./image/install_ui_osuse.png)
...
![Suggested Partitioning](./image/install_partitioner_osuse.png]](./image/install_partitioner_osuse.png)
...
![Installation Settings](./image/install_summary_osuse.png]](./image/install_summary_osuse.png)
...
![US keyboard layout](./image/keyboard_us.svg]](./image/keyboard_us.svg)
...
![YaST user and group administration](./image/yast2_users_main_kde.png]](./image/yast2_users_main_kde.png)
...
![YaST software manager screen](./image/yast2_sw_manager.png]](./image/yast2_sw_manager.png)
...
![Conflict management of the software manager](./image/yast2_package_conflict.png]](./image/yast2_package_conflict.png)
...
![GNOME Software—Updates view](./image/gnome-software-updates.png]](./image/gnome-software-updates.png)
...
[11.1 在线更新对话框](#sec-onlineupdate-you-overview)

[11.2 安装补丁](#sec-onlineupdate-you-install)

[11.3 查看撤回的补丁](#sec-onlineupdate-retracted-patches)

[11.4 自动在线更新](#sec-onlineupdate-you-automatically)
...
![YaST online update](./image/yast2_you_osuse.png]](./image/yast2_you_osuse.png)
...
![Viewing retracted patches and history](./image/yast2_retracted_patches.png]](./image/yast2_retracted_patches.png)
...
1.  安装后，启动 YaST 并选择 软件 › 在线更新。选择 配置 › 在线更新。如果未安装 yast2-online-update-configuration，系统将提示您安装它。
    ...
    ![YaST online update configuration](./image/yast2_autoupdate.png]](./image/yast2_autoupdate.png)
...
`mv` _OPTIONS_ _SOURCE_ _TARGET_
...
`cd` _OPTIONS_ _DIRECTORY_

更改当前目录。不带任何参数的 `cd` 会更改为用户的家目录。

`mkdir` _OPTIONS_ _DIRECTORY_

创建一个新目录。

`rmdir` _OPTIONS_ _DIRECTORY_

如果该目录已经为空，则删除指定的目录。

`chown` _OPTIONS_ `_USER_NAME_[:_GROUP_]` _FILES_
...
`chmod` _OPTIONS_ _MODE_ _FILES_

更改访问权限。

`mode` 参数包含三个部分：`group`、`access` 和 `access type`。 `group` 接受以下字符
...
`tar` _OPTIONS_ _ARCHIVE_ _FILES_
...
此命令可用于浏览指定文件的内容。使用 Page ↑ 和 Page ↓ 向上或向下滚动半页，或使用 Space 向下滚动整页。使用 Home 和 End 跳转到文件的开头或结尾。按 Q 退出程序。

`grep` _选项_ _搜索字符串_ _文件_
...
发送 _KILL_ 信号而不是 _TERM_ 信号，几乎在所有情况下都结束指定的进程
...
`host` _选项_ _主机名_ _服务器_

域名系统将域名解析为 IP 地址。使用此工具，向名称服务器（DNS 服务器）发送查询。

`ssh` _选项_ `[_用户_@]_主机名_` _命令_
...
[21.1 查找和收集信息](#sec-trouble-info)

[21.2 启动问题](#sec-trouble-boot)

[21.3 登录问题](#sec-trouble-login)

[21.4 网络问题](#sec-trouble-netconfig)

[21.5 数据问题](#sec-trouble-data)
...
![Select disk](./image/rescue_select_disk.png]](./image/rescue_select_disk.png)
